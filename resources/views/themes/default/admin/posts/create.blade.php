...
@section('title', '새 포스트 작성')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">새 포스트 작성</h1>
        ...
        <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="summary" 
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('summary') border-red-500 @enderror">{{ old('summary') }}</textarea>
                    @error('summary')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            <option value="archived" {{ old('status') == 'archived' ? 'selected' : '' }}>보관</option>
                        </select>
                        @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="seo_og_description" 
                                  ...
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('seo.og_description') }}</textarea>
                    ...
                    <div>
                        ...
                        <select id="seo_robots" name="seo[robots]" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            <option value="index, follow" {{ old('seo.robots') == 'index, follow' ? 'selected' : '' }}>인덱스 허용</option>
                            <option value="noindex, nofollow" {{ old('seo.robots') == 'noindex, nofollow' ? 'selected' : '' }}>인덱스 차단</option>
                        </select>
                        @error('seo.robots')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
            ...
            <div class="flex justify-end space-x-4">
                <a href="{{ route('admin.posts.index') }}" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 transition duration-200">
                    취소
                </a>
