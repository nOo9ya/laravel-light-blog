...
APP_USER="www-data"
DOMAIN=""
USE_SYSTEMD_TIMER=false  # true이면 systemd 타이머 사용, false이면 cron 사용
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --use-systemd-timer systemd 타이머 사용 (기본: cron 사용)"
            echo ""
            echo "예제:"
...
# Laravel 서비스 활성화 - 조건부 설정
if [ "$USE_SYSTEMD_TIMER" = true ]; then
    log_info "systemd 타이머로 Laravel Scheduler 설정"
    systemctl enable laravel-scheduler.timer
    systemctl start laravel-scheduler.timer
    log_success "systemd 타이머가 활성화되었습니다."
else
    log_info "cron을 사용하는 설정입니다. systemd 타이머는 비활성화됩니다."
    log_warning "Laravel Scheduler는 crontab에서 설정하세요: * * * * * cd $PROJECT_DIR && php artisan schedule:run"
fi
...
log_success "설정된 서비스:"
if [ "$USE_SYSTEMD_TIMER" = true ]; then
    log_success "✅ Laravel Scheduler (systemd 타이머, 매분 실행)"
else
    log_success "⚠️  Laravel Scheduler (cron 사용, 별도 설정 필요)"
fi
