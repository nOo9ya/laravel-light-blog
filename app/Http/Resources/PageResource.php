...
class PageResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        return [
            ...
            // URL
            'urls' => [
                'self' => route('api.pages.show', $this->slug),
                'web' => route('pages.show', $this->slug),
                'edit' => $this->when(
            ...
            'navigation' => $this->when(
                ...
                [
                    ...
                    'breadcrumb_trail' => $this->when(
                        ...
                        $this->ancestors->pluck('title')->push($this->title)->implode(' > ')
                    ),
                ]
            ),
    ...
    public function with($request)
    {
        return [
            'links' => [
                'siblings' => $this->parent_id ? route('api.pages.siblings', $this->id) : null,
                'children' => route('api.pages.children', $this->id),
                'related_pages' => route('api.pages.related', $this->id),
            ],
        ];
    }
}